...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_withheld', default='32', type=int)
    args = parser.parse_args()

    ############
    # Set up export directories
    data_dir = f'{args.root_dir}'
    vis_dir = f'{args.root_dir}/vis/res{args.width}_N_{args.num_t}_witheld_{args.num_withheld}'
    ...
    DC = torchvision.transforms.Resize([args.width, args.width])(torch.FloatTensor((np.load(f'{data_dir}/moving_digits_n256.npy') / 255.))).to(DEVICE)
    ...
    for i, t in enumerate(np.linspace(start=0, stop=1, num=args.num_t, endpoint=True)):
      ...
      true_turb_frames.append(np.uint8(255 * ang_to_unit(np.angle(g.cpu().detach().numpy()))))
    imageio.mimsave(f'{vis_dir}/final/gt_extrap_obs.gif', true_turb_frames[-args.num_withheld:], fps=30)
    imageio.mimsave(f'{vis_dir}/turbulence.gif', true_turb_frames, fps=60)
    ...
    for i in range(args.num_t):
      ...
      _y = F.conv2d(DC[i:(i+1)].unsqueeze(0), _kernel.unsqueeze(0), padding='same')
      uncor_ys.append(_y)
      obs_grayscale.append(np.uint8(255 * _y.squeeze().cpu().numpy()))
    ...
    imageio.mimsave(f'{vis_dir}/init_temporal.gif', obs_grayscale, fps=60)
    ...
    ############
    # Initialize network
    net = MovingTemporalZernNet(args.width, args.num_t, PSF_size)
    net = net.to(DEVICE)
    ...
    ############
    # Load data
    xs, ys = preprocess_sim_dynamic_data(DC, true_gs)
    dset = RandomDataset(xs[:-args.num_withheld], ys[:-args.num_withheld])
    train_loader = torch.utils.data.DataLoader(dset, batch_size=args.n_batch, num_workers=args.num_workers, shuffle=True)
    total_it = 0
    ...
    for epoch in t:
        for it, (x_batch, y_batch, idx) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            y, _kernel, sim_g, sim_phs, I_est = net(x_batch, idx / args.num_t - 0.5)
    ...
    for t in range(args.num_t):
        ...
        kernel = kernel / torch.sum(kernel, dim=[-2, -1], keepdim=True)
        y_conju_corrected = torch.clamp(F.conv2d(DC[t:(t+1)].unsqueeze(0), kernel.unsqueeze(0), padding='same').squeeze(), 0, 1)
        out_frames.append(np.uint8(y_conju_corrected.squeeze().detach().cpu().numpy() * 255))
    ...
    imageio.mimsave(f'{vis_dir}/final/final_turbulence.gif', out_errs, fps=60)
    imageio.mimsave(f'{vis_dir}/final/final_temporal.gif', out_frames, fps=60)
    imageio.mimsave(f'{vis_dir}/final/final_Iest.gif', out_Iest, fps=60)

    imageio.mimsave(f'{vis_dir}/final/extrap_Iest.gif', out_Iest[(-args.num_withheld):], fps=30)
    imageio.mimsave(f'{vis_dir}/final/gt_extrap_Iest.gif', DC[(-args.num_withheld):].detach().cpu().squeeze().numpy(), fps=30)
    imageio.mimsave(f'{vis_dir}/final/extrap_phs.gif', out_errs[(-args.num_withheld):], fps=30)
    imageio.mimsave(f'{vis_dir}/final/gt_extrap_phs.gif', true_turb_frames[(-args.num_withheld):], fps=30)
    imageio.mimsave(f'{vis_dir}/final/corrected_w_extrap_phs.gif', out_frames[(-args.num_withheld):], fps=30)
