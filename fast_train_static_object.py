...
if __name__ == "__main__":
    ...
    ############
    # Initialize network
    net = TemporalZernNet(args.width, PSF_size, args.num_t)
    net = net.to(DEVICE)
    ...
    x_batches=torch.zeros(args.num_t,1,args.width,args.width,dtype=torch.complex64).cuda()
    y_batches = torch.zeros(args.num_t, args.width, args.width,dtype=torch.float32).cuda()
    for i in range(args.num_t):
        x_batches[i,0,:,:]=torch.tensor(xs[i].cuda(),dtype=torch.complex64)#complex32 might be a lot faster
        y_batches[i,:,:]=ys[i].cuda()
    idxs=torch.tensor(np.arange(args.num_t),dtype=torch.float32).cuda()
    ...
    # Training loop
    for epoch in t:
        idxs=torch.randperm(args.num_t).to(DEVICE)
        for it in range(0,args.num_t,args.n_batch):
        # for it, (x_batch, y_batch, idx) in enumerate(train_loader):
            x_batch, y_batch, idx = x_batches[it:it+args.n_batch,:,:,:], y_batches[it:it+args.n_batch,:,:], idxs[it:it+args.n_batch]
